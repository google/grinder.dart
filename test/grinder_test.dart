...
void main() {
  group('grinder', () {
    ...
    test('badTaskName', () {
      ...
      expect(
        () => grinder.start(['bar'], dontRun: true),
        throwsA(isA<GrinderException>()),
      );
    });
    ...
    test('duplicate task name', () {
      ...
      grinder.addTask(GrinderTask('foo', taskFunction: nullTaskFunction));
      expect(
        () => grinder.start(['foo'], dontRun: true),
        throwsA(isA<GrinderException>()),
      );
    });
    ...
    test('throws when overwriting default task', () {
      ...
      expect(() {
        grinder.defaultTask = GrinderTask(
          'bar',
          taskFunction: nullTaskFunction,
        );
      }, throwsA(isA<GrinderException>()));
    });
    ...
    test('stringEscape', () {
      ...
      expect(
        grinder.getBuildOrder(),
        orderedEquals(
          ['b', 'a', 'd', 'c', 'e'].map((taskName) => TaskInvocation(taskName)),
        ),
      );
    });
    ...
    test('task execution order 1', () {
      ...
      grinder.addTask(
        GrinderTask('mode-notest', taskFunction: nullTaskFunction),
      );
      ...
      grinder.addTask(
        GrinderTask('deploy-test', depends: ['setup', 'mode-test']),
      );
      grinder.addTask(GrinderTask('docs', depends: ['setup']));
      grinder.addTask(
        GrinderTask('archive', depends: ['mode-notest', 'compile']),
      );
      grinder.addTask(
        GrinderTask('release', depends: ['mode-notest', 'compile']),
      );
